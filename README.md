...
4. The "Download PDF" button links to `pdfs/Invoice-<id>.pdf` — a real static file, one per invoice, pre-generated by `scripts/generate-invoice-pdfs.js` (mirrors the same -30..+30 day logic as the table). Each file's content is distinct (own invoice number/date/amount), and it's a real fetchable file rather than a shared static PDF or a client-side-only Blob download. Re-run the script (`cd scripts && npm install && node generate-invoice-pdfs.js`) to refresh dates if they drift too far from "today".
